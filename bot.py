...
GOOGLE_TOKEN    = os.environ.get('GOOGLE_TOKEN', '')   # JSON-строка токена
...
{{
  ...
  "cena":     "цена в $ (только число) или 'не указана'",
  ...
  "opisanie": "продающее описание для прайса на русском, 2–3 предложения"
        ...
        # ── Ссылка на фото ────────────────────────────────────────────────────
        photo_url = (
            f"https://api.telegram.org/file/bot{TELEGRAM_TOKEN}/{tg_file.file_path}"
        )

        # ── Запись в таблицу (столбцы A–F) ───────────────────────────────────
        # A: (пусто), B: Фото, C: Модель, D: Артикул, E: Цена $, F: Описание
        row = [
            "",
            photo_url,
            data.get("model", ""),
        ...
        await msg.edit_text(
            "⚠️ Не удалось разобрать ответ ИИ. Попробуй ещё раз или добавь подпись к фото."
        )
